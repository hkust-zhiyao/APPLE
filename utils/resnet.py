...
class ResNet600(nn.Module):
    def __init__(self, configs, bottleneck=False, num_classes=1000):
        super(ResNet600, self).__init__()
    ...
    def forward(self, x):
        #print ('ResNet600 in', x.shape)
        x = self.encoder(x)
        x = self.avpool(x)
        x = torch.flatten(x, 1)
        #print ('ResNet600 flat', x.shape)
        x = self.fc(x)
        #print ('ResNet600 out', x.shape)
        return x
...
def test():
    ...
    print ('configs, bottleneck', configs, bottleneck)
    encoder = ResNetEncoder(configs, bottleneck)
    #input = torch.randn((5,3,224,224))
    input = torch.randn((5,1,224,224))
    print(input.shape)
    output = encoder(input)
    print('test, output of encoder', output.shape)
    decoder = ResNetDecoder(configs[::-1], bottleneck)
    output = decoder(output)
    print('test, output of decoder', output.shape)


def test2():
    print ('test2')
    configs, bottleneck = get_configs("resnet152")
    net = ResNet(configs, bottleneck, num_classes=1)
    input = torch.randn((5,1,224,224))
    print(input.shape)
    output = net(input)
    print(output.shape)
...
def getModel(name='resnet18'):
    configs, bottleneck = get_configs(name)
    encoder = ResNetEncoder(configs, bottleneck)
    decoder = ResNetDecoder(configs[::-1], bottleneck)
    return encoder, decoder

def getModelNet(name='resnet18'):
    configs, bottleneck = get_configs(name)
    net = ResNet(configs, bottleneck, num_classes=1)
    return net


def getModelNet600(name='resnet18'):
    configs, bottleneck = get_configs(name)
    net = ResNet600(configs, bottleneck, num_classes=1)
    return net
