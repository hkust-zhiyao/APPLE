...
def plots(x, y, name, c, m='o', s=13, ls='-', zo=0):
    ...
    plt.plot(x, y, c=c, ls=ls, linewidth=3.5,
             marker=m, markersize=s, label=name + '\n' + r'$(k=' + k + ', R=' + r+ ')$' , zorder=zo)
             #marker=m, markersize=s, label=name + r' $(R='+r+')$', zorder=zo)
...
def signal():
    ...
    sout_l = [0.2790, 0.2734, 0.2632, 0.2402] # to plot
    win_l  = [0.1071, 0.1251, 0.1270, 0.1601]

    sin_a  = [0.1222, 0.1588, 0.2498, 0.2800]
    sout_a = [0.4041, 0.3417, 0.3282, 0.1803] # to plot
    ...
    plots(x, sout_l,  name= r'$FC_{out}$' + '_LIME' + r'$\downarrow$', c='orange', m='s', s=12, ls=':')
    plots(x, sout_a,  name= r'$FC_{out}$' + '_APPLE' + r'$\downarrow$', c="xkcd:" + 'turquoise', m='s', s=12)
    ...
    #plt.legend(fontsize=19, ncol=2, columnspacing=0.8, 
    #           labelspacing=0.3, borderaxespad=0.2, loc='upper right')
    legend = plt.legend(fontsize=19, ncol=1, columnspacing=0.5,
               labelspacing=1, borderaxespad=0.2, loc='upper right')
    ...
    plt.savefig('FOC_curve2', dpi=200)
    print ('save:', 'FOC_curve2')
    plt.close('all')
    plt.clf()
